      ...
      <div className="product-frame w-full max-w-md p-2">
        <div className="product-frame-inner flex-col !aspect-auto py-12 px-8 shadow-2xl">
          ...
          <div className="text-center mb-10">
            <h2 className="text-3xl font-cinzel text-vintage-contrast mb-2 tracking-widest">{t('sign_in')}</h2>
            ...
            <p className="text-vintage-contrast/80 font-lora text-sm">{t('welcome_back')}</p>
          </div>

          {error && (
            <div className="mb-6 p-3 bg-red-900/10 border border-red-900/30 text-red-800 text-xs text-center font-lora">
              {error}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="w-full flex flex-col gap-6">
            ...
            <div className="mt-8 pt-8 border-t border-vintage-gold/20 flex flex-col items-center">
              <p className="text-[10px] uppercase tracking-widest text-vintage-contrast/60 mb-4 font-cinzel">Quick Access (Seeded Accounts)</p>
              <div className="flex gap-4 w-full">
                <button
                  type="button"
                  onClick={() => handleQuickLogin('admin@collector.shop')}
                  className="flex-1 text-[10px] py-2 border border-vintage-gold/40 text-vintage-contrast hover:bg-vintage-gold/10 transition-colors uppercase tracking-[0.2em] font-cinzel"
                >
                  Admin
                </button>
                <button
            ...
            <p className="text-center text-vintage-contrast/70 text-xs font-lora mt-4">
              {t('new_here')} <Link to="/register" className="text-vintage-contrast font-bold hover:underline">{t('create_account_link')}</Link>
            </p>
